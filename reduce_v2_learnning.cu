...
__global__ void reduce_v0(float* d_in,float* d_out){
    ...
    for (unsigned int index = blockDim.x / 2; index > 0; index >>= 1) {
        if (tid < index) {
            ...
            //解决sharedmemory的bank conflict问题
            //>>1是位运算，除2
            //index>>=1等价于 index/=2
            //第一轮 index = 128，shared memory有32列座位， tid = 0在第一排，tid+index = 128在第五排
            //第二轮 index = 64，shared memory有32列座位，tid = 0在第一排，tid+index在第3排
            //第三轮 index = 32，sahred memory有32列座位，tid = 0在第一排，tid+index在第二排

            //这样可以保证shared memory的字段是并行访问的都不是串行访问的。
        }
        __syncthreads();
    }
